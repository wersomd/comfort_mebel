...
import { minVariantPrice, hasPriceRange, variantLabel } from "../lib/variants";
...
function isAvailable(p: Product, color?: ProductColor): boolean {
  ...
  if (p.colors && p.colors.length > 0)
    return p.colors.some((c) => c.stock == null || c.stock > 0);
...
export function ProductCard({ product, index = 0 }: Props) {
  ...
  const images = useMemo(() => {
    const fromColor =
      activeColor && activeColor.images.length > 0 ? activeColor.images : null;
  ...
  const handleAdd = (e: React.MouseEvent) => {
    // Если есть размеры — нужно выбрать на странице товара: даём ссылке отработать
    if (hasSizes) return;
    e.preventDefault();
    e.stopPropagation();
    if (!available) return;
    add(product, {
      ...
      variantLabel: variantLabel(activeColor, undefined),
    });
    setAdded(true);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Hover overlay */}
        <div
          className="absolute inset-0 bg-[#3D2C25]/10 transition-opacity duration-300"
        ...
        {images.length > 1 && (
          <>
            ...
            <div
              className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5 transition-opacity duration-200"
            ...
            >
              {images.map((_, i) => (
                <span
                  ...
                  style={{
                    background:
                      i === imgIndex ? "#FFFFFF" : "rgba(255,255,255,0.55)",
        ...
        >
          {!available ? "Нет в наличии" : hasSizes ? "Выбрать" : added ? "✓ Добавлено" : "В корзину"}
        </button>
      </div>

      {/* Text */}
      <div>
        <p className="text-[10px] tracking-[1.5px] uppercase text-[#9A8070] font-['Inter'] mb-1">
          {product.sku}
        </p>
        ...
        {(activeColor?.material || product.material) && (
          <p className="text-[12px] text-[#9A8070] mb-1">
            {activeColor?.material || product.material}
          </p>
        )}
        ...
        {colors.length > 0 && (
          <div className="flex items-center gap-1.5 mt-2 mb-1.5">
            ...
            {colors.length > 6 && (
              <span className="text-[10px] text-[#9A8070] font-['Inter'] ml-1">
                +{colors.length - 6}
              </span>
            )}
          </div>
        ...
        <div className="flex items-baseline gap-2">
          {hasPriceRange(product) ? (
            <span className="text-[14px] font-['Inter'] font-semibold text-[#3D2C25]">
              от {formatPrice(minVariantPrice(product))}
            </span>
          ) : (
            <>
              <span className="text-[14px] font-['Inter'] font-semibold text-[#3D2C25]">
              ...
              {product.oldPrice && (
                <span className="text-[12px] text-[#9A8070] line-through">
